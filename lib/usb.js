...
USBDevice.prototype._write = function(chunk, encoding, callback) {
  var self = this;
  console.log("===> writing to endpoint");
  self.output.transfer(chunk, function(err) {
    callback(err);
  });
};

USBDevice.prototype._read = function(length) {
  var self = this;
  console.log("===> reading from endpoint");
  self.input.transfer(length, function(err, data){
    self.push(data);
  });
};
...
USBDevice.prototype.start = function() {
  var self = this;
  self.iface.claim();
  self.input.startStream(3, 512);
  self.output.startStream(3, 512);
  self.emit('connected');
};

USBDevice.is_adb_available = function(device, callback) {
  device.open();
  ...
  if( (device.deviceDescriptor !== null) && (device.configDescriptor !== null)) {
    ...
    device.interfaces.forEach(function(iface) {
      if(iface.endpoints.length == 2) {
        if(USB_VENDOR_IDS.indexOf(vid) != -1){
          if(
